...
define config.name = _("JAPN 300 Final Project")
...
## 実行ファイルやビルドされた配布物のディレクトリー名に使われる、ゲームの簡易名。
## 簡易名は ASCII 文字（半角英数字）のみで構成され、スペース・コロン・セミコロ
## ンなどを含んでは行けません。

define build.name = "JAPN300FinalProject"
...
define config.save_directory = "JAPN300FinalProject-1729660154"
...
init python:
    ...
    ## "game/**.ogg" はゲームディレクトリー及びそのサブディレクトリーにある全
    ## ての ogg ファイルに一致し、"**.psd" はプロジェクトのあらゆる psd ファイル
    ## に一致します。
